...
const selectors = {
  ...
  connectButtonMaster: `.ph5.pb5 *[aria-label="Invite $# to connect`,
  messageButton: ".ph5 .pvs-profile-actions .entry-point",
  messageButtonUpdated: ".ph5 .pvs-profile-actions button[aria-label='Message #$']",
...
const SendConnectionRequest = async (profiles, messageTemplate, headless = true) => {
  ...
  const connect = async (profile) => {
    ...
    const getConnectionStatus = async () => {
      ...
      const connectionStatus = await page.evaluate((selectors) => {
        ...
        if (element.length == 2) {
          ...
          if (element[0].innerText == "Following" || element[0].innerText == "Follow") return 2;
          return 3;
        }
      }, selectors);
    ...
    if (connectionStatus != 1) {
      ...
      //   await requestWithNote();
      // }
      const connectButton = await page.waitForSelector(selectors.connectButtonMaster);
      await connectButton.click();
      await requestWithNote();
    } else {
  ...
  const message = async (profile) => {
    ...
    const messageButton = await page.waitForSelector(
      selectors.messageButtonUpdated.replace("#$", profile.fullName.split(" ")[0])
    );

    console.log(messageButton);
    await new Promise((r) => setTimeout(r, 1000));

    console.log(` selectors.messageButtonUpdated.replace("#$", profile.fullName.split(" ")[0]`);
    ...
    const messageForm = await page.waitForSelector(`.msg-form__contenteditable[role="textbox"]`);
    ...
    await page.evaluate((messageNote) => {
      document.querySelector(`.msg-form__contenteditable[role="textbox"] p`).innerHTML =
        messageNote;
    }, messageNote);
    // await page
    await new Promise((r) => setTimeout(r, 1000));
  ...
  for (let i = 0; i < profiles.length; i++) {
    ...
    selectors.connectButtonMaster = selectors.connectButtonMaster.replace(
      "$#",
      profiles[i].fullName
    );
